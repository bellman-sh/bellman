 ...
 *
 * - off     The webhook refuses and /upgrade serves no links. Grants Stripe
 *           has already written stop being honoured; grants an operator or an
 *           admin wrote are untouched, so this disables *billing*, not plans.
 *           The default, and what any value other than the three below means.
 * - shadow  The webhook records payments and writes the grants, and /upgrade
 *           sends people to pay, but tokens ignore anything Stripe granted.
 *           For taking real test purchases end to end, and checking the
 *           ledger, before anyone's plan depends on it. Writing the grants is
 *           deliberate: it is what makes switching to `on` activate purchases
 *           already seen, instead of waiting for Stripe to mention them again.
