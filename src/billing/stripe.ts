...
/**
 * Make the stored grant match the ledger, and say what happened.
 *
 * A clash with a hand-written grant is not a delivery failure — the event was
 * processed correctly, a human has a decision to make — so it is logged and
 * acknowledged rather than retried forever. A storage failure is different and
  ...
  if (outcome === "conflict") {
    console.error(
      `stripe ${eventId}: ${userId} has a plan granted by hand, so their purchase was not applied. ` +
        `Resolve it in /admin/grants.`
    );
  }
  if (outcome === "unkeyable") {
    console.error(`stripe ${eventId}: ${userId} is not a provider-derived user id; no grant written`);
  }
  return outcome;
}
...
export async function handleStripeWebhook(request: Request, config: StripeWebhookConfig): Promise<Response> {
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      // Linking can be what makes an already-recorded subscription count, so
      // the grant is rewritten here too, not only on subscription events.
      const applied = linked ? await settle(userId, config, event.id) : "conflict";
      return reply(200, { received: true, applied: linked, grant: applied });
    }
    ...
    case "customer.subscription.resumed": {
      ...
      // An event names one subscription; the plan is the best of all of them,
      // so the answer is recomputed from the ledger rather than from this event.
      const owner = await config.billing.userForCustomer(customerId);
      if (!owner) {
        // Stripe delivers out of order: the checkout that links this customer
        // may still be coming, and it reconciles when it lands.
        console.warn(`stripe ${event.id}: customer ${customerId} is not linked to a user yet`);
        return reply(200, { received: true, applied: true, grant: "unlinked" });
      }
      return reply(200, { received: true, applied: true, grant: await settle(owner, config, event.id) });
    }
